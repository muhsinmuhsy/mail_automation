...
const getOptions = defineRoute(async (_req, ctx) => {
  ...
  const [accounts, attachments, contacts] = await Promise.all([
    ...
    db.contact.findMany({ where, select: { id: true, name: true, email: true }, orderBy: { name: 'asc' } }),
  ]);
  const response = respondOk({
    ...
    templates: [],
    contacts: contacts.map(contact => ({ id: contact.id, label: contact.name, description: contact.email })),
  }, ctx.requestId);
