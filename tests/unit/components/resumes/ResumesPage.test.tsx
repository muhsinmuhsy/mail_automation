...
describe('ResumesPage uploads', () => {
  it.each(['api', 'network'])('displays %s failures and lets the user retry', async (failure) => {
    ...
    if (failure === 'api') {
      fetchMock.mockResolvedValueOnce({ ok: false, json: async () => ({ success: false, error: { message: 'Resume must be a valid PDF file.' } }) });
    } else {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent(failure === 'api' ? 'Resume must be a valid PDF file.' : 'Network unavailable.');
    await user.click(screen.getByRole('button', { name: 'Upload' }));
    expect(await screen.findByText('cv.pdf')).toBeInTheDocument();
    expect(screen.queryByRole('alert')).not.toBeInTheDocument();
    expect(fetchMock.mock.calls[1][0]).toBe('/api/resumes');
